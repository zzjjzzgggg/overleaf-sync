...
@click.option(
    ...
    type=click.Path(exists=False),
    help=
    "Relative path of the .olignore file (ignored if sync from remote to local).")
@click.option('-f',
              '--folder_id',
              'folder_id_path',
              default=".folder_id",
              type=click.Path(exists=False),
              help="Relative path of the .folder_id file.")
@click.pass_context
        ...
        if remote or sync:
            sync_func(
                ...
                from_newer_than_to=lambda name: dateutil.parser.isoparse(project[
                    "lastUpdated"]).timestamp() > os.path.getmtime(
                        os.path.join(sync_path, name)),
                from_name="remote",
                to_name="local")
        if local or sync:
            sync_func(
                ...
                create_file_at_to=lambda name: overleaf_client.upload_file(
                    project["id"], get_folder_id(project, folder_id_path), name,
...
def login(username, password, cookie_path):
    ...
    click.clear()
    execute_action(
        lambda: login_handler(username, password, cookie_path), "Login",
...
def sync_func(files_from, create_file_at_to, from_exists_in_to, from_equal_to_to,
              from_newer_than_to, from_name, to_name):
...
def get_folder_id(project, folder_id_path):
    if os.path.isfile(folder_id_path):
        with open(folder_id_path, 'r') as f:
            return f.readline().rstrip()
    else:
        return format(int(project['id'], 16) + 2, 'x')
